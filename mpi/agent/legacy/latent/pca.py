...
datasets = ["D4RL/pointmaze/umaze-dense-v2",'D4RL/pointmaze/open-dense-v2','D4RL/pointmaze/medium-dense-v2']
all_trajectories = []
...
sampler = StratifiedSampler(dataset, N_per_class=500)
...
reduced_h_sequences = pca.fit_transform(np.array(all_h_sequences))

# Plot the results
plt.figure(figsize=(10, 8))

dataset_colors = {
    "D4RL/pointmaze/umaze-dense-v2": "r", 
    "D4RL/pointmaze/open-dense-v2": "g", 
    "D4RL/pointmaze/medium-dense-v2": "b"
}

used_labels = set()

for i, (x, y) in enumerate(reduced_h_sequences):
    name = dataset_names[i]
    color = dataset_colors[name]
    size = total_rewards[i] / max(total_rewards) * 100

    label = name if name not in used_labels else None
    plt.scatter(x, y, c=color, s=size, alpha=0.5, label=label)
...
plt.title("PCA of h_sequence from Decision Transformer Encoder")
plt.show()

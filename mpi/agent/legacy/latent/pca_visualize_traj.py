...
sampler = StratifiedSampler(dataset, N_per_class=3000)
...
from plot_sequence import plot_desired_goal_sequences


while True:
    indexs_str = input("input your list")
    try:
        indexs = eval(indexs_str)
        def plot_sequences(indexs):
            plot_desired_goal_sequences([desired_goal_sequences[i].numpy() for i in indexs])
        plot_sequences(indexs=indexs)
    except:
        print("ERROR")
        continue;

from data.dataloader import MinariSequenceDataset
from torch.utils.data import DataLoader
def process_dataloader(env_name,args):
    if env_name == 'kitchen_mixed-v2':
        dataset = MinariSequenceDataset(env_name = 'D4RL/kitchen/mixed-v2', context_len=args.context_len)
        ...
        dataset = MinariSequenceDataset(env_name = 'D4RL/kitchen/complete-v2', context_len=args.context_len)
        dataloader = DataLoader(dataset, batch_size=args.training["batch_size"], shuffle=args.training["shuffle"], num_workers=args.training["num_workers"])
    else:

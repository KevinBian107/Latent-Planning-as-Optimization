...
for t in range(STEP):
    ...
    states = torch.tensor([list(state_buffer)], dtype=torch.float32)
    actions = torch.tensor([list(action_buffer)], dtype=torch.float32)
    rewards = torch.tensor([0], dtype=torch.float32).reshape(1,1)
    timesteps = torch.tensor([list(timestep_buffer)], dtype=torch.long)

    # Forward pass
    pred_action, _, _ = model(states, actions, timesteps, rewards, batch_inds = torch.tensor([0])) #batch_inds can be any tesnor actually as it won't be used in eval
    action = pred_action.squeeze().detach().numpy()  # last token prediction
    ...
    # Update context
    state_buffer.append(next_obs["observation"])
